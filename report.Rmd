```{r}
# Clean the objects of the workspace
rm(list = ls())
...
summary(heart_failure_preprocessed[continuous_vars_preprocessed])
...
ggplot(tune_grid, aes(x = factor(mtry), y = OOB_Error, fill = factor(nodesize))) +
...
ggplot(metrics_melt, aes(x = Threshold, y = value, color = variable)) +
...
ggplot(compare_melt, aes(x = Model, y = as.numeric(value), fill = variable)) +
